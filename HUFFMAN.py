...
class Image:
    ...
    def upTree(self):
        ...
        while 1:
            ...
            workspace = list(
                heapq.merge(workspace, [new_node], key=self.prob_key)
            )  # Pushing the created Node into Workspace
    ...
    def decode(self, path):
        ...
        for i, c_int in enumerate(self.binaryFromFile):
            if (
                ...
                and temp_c != 0
                and temp_d != 0
                and len(decodeList) == (temp_r * temp_c * temp_d + 3)
            ):
                break
    ...
    def huffmanCode(
        ...
        print(
            "\nCompressed factor : ",
            self.r * self.c * self.d * 8 / (len(self.encodedString)),
            "\n",
        )
...
name = "p.jpg"  # change the name to the image you want to applay huffaman code on it
rname = "./" + name
image = Image()
image.huffmanCode(rname, "./ompressed.bin", "./output.png", toCheck=1)

# after this code finsh you will get tow files on compressed.bin that have the encoded image and the the other output.png is the image after it decoded it
